...
void I2C_GPIO_Init(void) {
	...
	// Set mode of Pin6 and 7  to Alt. Function (clear first)
	...
	GPIOB->PUPDR &= ~(GPIO_PUPDR_PUPD6);//pull up resistors
...
void I2C_Initialization(void){
	...
	RCC->APB1ENR1 |= RCC_APB1ENR1_I2C1EN;//enable i2c1 clk
	RCC->APB1ENR1 &= ~RCC_CCIPR_I2C1SEL; //clear the bits
	RCC->CCIPR |= RCC_CCIPR_I2C1SEL_1;//set clock to i2c1
	
	RCC->APB1RSTR1 |= (RCC_APB1RSTR1_I2C1RST);//reset mode
	RCC->APB1RSTR1 &= ~(RCC_APB1RSTR1_I2C1RST);//cleared so it doesnt remain in stater
	
	I2C1->CR1 &= ~(I2C_CR1_PE); //Disable I2C before we start modifying
	
	I2C1->CR1 &= ~(I2C_CR1_ANFOFF);//Enable analog noise filter
	
	I2C1->CR1 &= ~(I2C_CR1_DNF);//Disable the digital noise filter
	
	I2C1->CR1 &= ~(I2C_CR1_ERRIE);//clear
	I2C1->CR1 |= I2C_CR1_ERRIE;//set
	...
	//low clk period = 4.7 micro seconds
	//high clk = 4.0 micro seconds
	//set up time = 1000ns
	//hold time = 1250 ns
	I2C1->TIMINGR |= 39U << I2C_TIMINGR_PRESC_POS;
	I2C1->TIMINGR |= 13U << I2C_TIMINGR_SCLDEL_POS;
	I2C1->TIMINGR |= 16U << I2C_TIMINGR_SDADEL_POS;
	I2C1->TIMINGR |= 53U << I2C_TIMINGR_SCLH_POS;
	I2C1->TIMINGR |= 62U << I2C_TIMINGR_SCLL_POS;
	
	
	I2C1->OAR1 &= ~(I2C_OAR1_OA1EN); //disable address 1
	I2C1->OAR2 &= ~I2C_OAR2_OA2EN; //ensure that own address 2 is disabled
	I2C1->OAR1 &= ~(I2C_OAR1_OA1MODE);//sets to 7-bit addressing(default if 10 is cleared)
	
	I2C1->OAR1 |= OwnAddr << 1;  //set to own address

	I2C1->OAR1 |= (I2C_OAR1_OA1EN);//enable address 1
	
	I2C1->CR1 |= (I2C_CR1_PE);//I2C enable i2c in the control register
}
